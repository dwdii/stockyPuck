...
The `monteCarloIteration` function is structured with the capability to predict prices \(n\) days in the future and could also be used for a crude value-at-risk computation. For our purposes, we only sample and walk 1 day into the future (`mcFutureDays` \(= 1\)), thereby returning tomorrow's precicted price for the given Monte Carlo iteration.
...
An our t-score for this hypothesis test is computed as:
